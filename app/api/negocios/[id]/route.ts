...
export async function PUT(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const [result] = await pool.query(
      ...
      [nombre_negocio, params.id]
    );
    if ((result as any).affectedRows === 0) {
      return NextResponse.json({ error: 'Negocio no encontrado' }, { status: 404 });
    }
    // Obtener el negocio actualizado
    const [rows] = await pool.query('SELECT * FROM negocios WHERE id_negocio = ?', [params.id]);
...
export async function DELETE(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    try {
      const [result] = await pool.query('DELETE FROM negocios WHERE id_negocio = ?', [params.id]);
      if ((result as any).affectedRows === 0) {
        return NextResponse.json({ error: 'Negocio no encontrado' }, { status: 404 });
      }

...
export function CumplidoNegocioTable({ negocioId, negocioNombre }: CumplidoNegocioTableProps) {
  ...
  const [contextMenu, setContextMenu] = useState<{ visible: boolean; x: number; y: number; colKey: string | null }>({ visible: false, x: 0, y: 0, colKey: null });
  const [notaDialog, setNotaDialog] = useState<{ open: boolean; idPuesto: number | null; colKey: string | null; nota: string }>({ open: false, idPuesto: null, colKey: null, nota: '' });
  const [horasEditadas, setHorasEditadas] = useState(false);
  const [showConfirmDialog, setShowConfirmDialog] = useState(false);
  const [pendingAction, setPendingAction] = useState<null | (() => void)>(null);
  const [turnoFiltro, setTurnoFiltro] = useState<'diurno' | 'nocturno' | '24h'>(getDefaultTurnoFiltro());
  ...
  const cargarCumplidos = useCallback(async () => {
    ...
    try {
      ...
      const grouped: Record<number, { diurno?: Cumplido; nocturno?: Cumplido }> = {};
      const nuevasHorasColumnas: { [key: string]: string } = {};
      reportes.forEach((reporte) => {
        let calificaciones: { [key: string]: number } = {};
        if (reporte.calificaciones && typeof reporte.calificaciones === 'object') {
          Object.entries(reporte.calificaciones).forEach(([rKey, horaObj]) => {
            if (horaObj && typeof horaObj === 'object') {
              const horas = Object.entries(horaObj);
              if (horas.length > 0) {
                const [hora, valor] = horas[0];
                let tipo: 'diurno' | 'nocturno' = 'diurno';
                if (reporte.id_tipo_turno === 2) tipo = 'nocturno';
                const key = `${rKey}_${tipo}`;
                calificaciones[key] = valor as number;
                nuevasHorasColumnas[key] = hora;
              }
            }
          });
        }
        const tipoTurno: 'diurno' | 'nocturno' = (reporte.id_tipo_turno === 2) ? 'nocturno' : 'diurno';
  ...
  const fechaSeleccionada = new Date(parseInt(date.anio), parseInt(date.mes) - 1, parseInt(date.dia));
  const { cantidad_diurno, cantidad_nocturno } = getApplicableConfig(fechaSeleccionada);

  const horas: { key: string; label: string; tipo: 'diurno' | 'nocturno' }[] = [
    ...Array.from({ length: cantidad_diurno }, (_, i) => ({
      key: `R${i + 1}_diurno`,
      label: `R${i + 1} (Diurno)` ,
      tipo: 'diurno' as const
    })),
    ...Array.from({ length: cantidad_nocturno }, (_, i) => ({
      key: `R${i + 1}_nocturno`,
      label: `R${i + 1} (Nocturno)`,
  ...
  const saveBatch = useCallback(async (changes: PendingChange[], horasCol: { [key: string]: string }) => {
    setSaving((prev) => {
    ...
    try {
      ...
      for (const change of changes) {
        ...
        if (cumplido?.diurno) {
          ...
          Object.keys(calificacionesDiurno).forEach(rKey => {
            if (Object.keys(calificacionesDiurno[rKey]).length === 0) {
              delete calificacionesDiurno[rKey];
            }
          });
        ...
        if (cumplido?.nocturno) {
          ...
          Object.keys(calificacionesNocturno).forEach(rKey => {
            if (Object.keys(calificacionesNocturno[rKey]).length === 0) {
              delete calificacionesNocturno[rKey];
            }
          });
          payloads.push({ id_cumplido: cumplido.nocturno.id_cumplido, calificaciones: calificacionesNocturno });
        }
      }
  ...
  }, [date, cargarCumplidos, cumplidos, horasFiltradas]);
          ...
          if ((!value || value === 0) && (!nota || nota.length === 0)) {
            ...
            if (newCalificaciones[key]) {
              ...
              horasKeys.forEach((hora) => {
                delete newCalificaciones[key][hora];
              });
            }
            newMap.set(idPuesto, {
              ...existing,
  ...
  const handleContextMenu = useCallback(
    (e: React.MouseEvent, idPuesto: number, tipo: 'diurno' | 'nocturno', hora: string) => {
      ...
      setModal({
        type: 'nota',
        open: true,
        config: { idCumplido: cumplido?.diurno?.id_cumplido || cumplido?.nocturno?.id_cumplido, idPuesto, tipo, hora, nota },
      });
    },
    [cumplidos]
  );
  ...
  const guardarNota = useCallback(async () => {
    ...
    try {
      const fecha = new Date(parseInt(date.anio), parseInt(date.mes) - 1, parseInt(date.dia));
      let idCumplido = cumplidos[modal.config.idPuesto]?.diurno?.id_cumplido || cumplidos[modal.config.idPuesto]?.nocturno?.id_cumplido;
      if (!idCumplido) {
  ...
  const guardarConfiguracion = useCallback(async () => {
    if (!modal.config?.fecha_inicial || modal.config.cantidad_diurno === undefined || modal.config.cantidad_nocturno === undefined) return;
    try {
  ...
  const handleNotaContextMenu = (e: React.MouseEvent, idPuesto: number, colKey: string) => {
    ...
    if (pending?.calificaciones?.[colKey] && typeof pending.calificaciones[colKey] === 'object' && 'nota' in pending.calificaciones[colKey]) {
      notaActual = (pending.calificaciones[colKey] as any).nota || '';
    } else {
      const cal = cumplidos[idPuesto]?.diurno?.calificaciones?.[colKey] || cumplidos[idPuesto]?.nocturno?.calificaciones?.[colKey];
  ...
  const handleGuardarNota = () => {
    ...
    setPendingChanges((prev) => {
      ...
      const calificaciones = existing.calificaciones || {};
      let valor = calificaciones[colKey];
  ...
  return (
    <div className="max-w-[1600px] mx-auto h-[90dvh] max-h-[90dvh] overflow-hidden">
      <Card className="w-full h-full flex flex-col">
          ...
          <div className="flex flex-row gap-2 mt-4 w-full justify-start items-end">
            <div className="flex flex-col items-start w-20">
              ...
              <Select value={date.anio ?? ''} onValueChange={(value) => handleChangeFecha('anio', value)}>
                <SelectTrigger className="w-20"><SelectValue placeholder="Año" /></SelectTrigger>
                <SelectContent>
                  {Array.from({ length: 13 }, (_, i) => ({ value: (new Date().getFullYear() + i).toString(), label: (new Date().getFullYear() + i).toString() }))
            ...
            <div className="flex flex-col items-start w-20">
              ...
              <Select value={date.mes ?? ''} onValueChange={(value) => handleChangeFecha('mes', value)}>
                <SelectTrigger className="w-20"><SelectValue placeholder="Mes" /></SelectTrigger>
                <SelectContent>
                  {Array.from({ length: 12 }, (_, i) => ({ value: (i + 1).toString().padStart(2, '0'), label: new Date(0, i).toLocaleString('es', { month: 'long' }) }))
            ...
            <div className="flex flex-col items-start w-20">
              ...
              <Select value={date.dia ?? ''} onValueChange={(value) => handleChangeFecha('dia', value)}>
                <SelectTrigger className="w-20"><SelectValue placeholder="Día" /></SelectTrigger>
                <SelectContent>
                  {Array.from({ length: 31 }, (_, i) => ({ value: (i + 1).toString().padStart(2, '0'), label: (i + 1).toString() }))
                ...
                <div className="space-y-2">
                  {Array.from({ length: 5 }).map((_, i) => (
                    <div key={`skeleton-${i}`} className="flex gap-2">
                      ...
                      <Skeleton className="h-12 w-[200px]" />
                      {Array(cantidad_diurno + cantidad_nocturno).fill(0).map((_, idx) => <Skeleton key={`skeleton-${i}-${idx}`} className="h-12 w-[100px]" />)}
                    </div>
                  ))}
                </div>
              ) : (
                <Table className="min-w-max w-full h-full" style={{ tableLayout: 'auto' }}>
                  <TableHeader className="sticky top-0 z-20" style={{ position: 'sticky', top: 0, background: 'white' }}>
                    <TableRow>
                      ...
                      {['diurno', '24h'].includes(turnoFiltro) && (
                        <TableHead className={`w-[200px] sticky left-[224px] z-30 bg-white`}>Nombre Colaborador Diurno</TableHead>
                      )}
                      {['nocturno', '24h'].includes(turnoFiltro) && (
                        <TableHead className={`w-[200px] sticky left-[${turnoFiltro === '24h' ? '424px' : '224px'}] z-30 bg-white`}>Nombre Colaborador Nocturno</TableHead>
                      )}
                      {horasFiltradas.map((col, idx) => {
                        const n = col.tipo === 'diurno'
                          ? idx - horasFiltradas.findIndex(h => h.tipo === 'diurno') + 1
                          : idx - horasFiltradas.findIndex(h => h.tipo === 'nocturno') + 1;
                        const sufijo = col.tipo === 'diurno' ? 'D' : 'N';
                            ...
                            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '100%' }}>
                              ...
                              {editMode && (
                                <div className="flex gap-2 mt-1">
                                        ...
                                        setPendingChanges((prev) => {
                                          ...
                                          if (calificaciones[reporteKey]) {
                                            ...
                                            if (horasKeys.length === 1 && horasKeys[0] !== nuevaHora) {
                                              calificaciones[reporteKey][nuevaHora] = calificaciones[reporteKey][horasKeys[0]];
                                  ...
                                  >
                                    <SelectTrigger className={`w-[60px]${editMode && !hora ? ' border border-red-500' : ''}`}>
                    ...
                    {(() => {
                      ...
                      Object.entries(unidades).forEach(([nombre_unidad, puestos]) => {
                        puestos.forEach((puesto, idx) => {
                          ...
                          const nombreNocturno = cumplidoNocturno?.colaborador_nocturno || cumplidoNocturno?.colaborador || '';
                          rows.push(
                            <TableRow key={`puesto-${puesto.id_puesto}`}>
                              {idx === 0 && (
                                <TableCell rowSpan={puestos.length} className="w-[24px] align-middle vertical-text">{nombre_unidad}</TableCell>
                              )}
                              ...
                              {horasFiltradas.map((col) => {
                                const cal = (pendingChanges.get(puesto.id_puesto)?.calificaciones
                                  || (col.tipo === 'diurno' ? cumplidoDiurno?.calificaciones : cumplidoNocturno?.calificaciones)
                                  || {});
                                const keyStr = String(col.key);
                                const valorObj = cal[keyStr];
                                const valor = (typeof valorObj === 'object' && valorObj !== null && 'valor' in valorObj)
                                  ? valorObj.valor
                                  : (typeof valorObj === 'object' ? '' : (valorObj ?? ''));
                                const tieneNota = typeof valorObj === 'object' && valorObj !== null && 'nota' in valorObj && valorObj.nota && valorObj.nota.length > 0;
                                const nota = tieneNota ? valorObj.nota : '';
                                const cell = (
                                  <TableCell key={col.key} className={"w-[140px] text-center " + (tieneNota ? 'bg-red-100' : '')}>
                                    {editMode ? (
                                      <Input
                                        type="number"
                                        min="0"
                                        ...
                                        onChange={e => handleChange(puesto.id_puesto, `calificaciones.${keyStr}`, parseInt(e.target.value || '0', 10) || 0)}
                                        onContextMenu={e => handleNotaContextMenu(e, puesto.id_puesto, keyStr)}
                                      />
                                    ) : (
                                      valor === 0 && tieneNota ? '' : valor
                                    )}
                                  </TableCell>
                                );
        ...
        <Dialog open={modal.type === 'nota' && modal.open} onOpenChange={(open) => setModal((prev) => ({ ...prev, open, type: open ? prev.type : null }))}>
          <DialogContent>
            <DialogHeader>
                  ...
                  <Select value={excelDesdeAnio} onValueChange={setExcelDesdeAnio}>
                    ...
                    <SelectContent>
                      {Array.from({ length: 13 }, (_, i) => {
                        ...
                        return <SelectItem key={`desde-anio-${year}`} value={year.toString()}>{year}</SelectItem>;
                      })}
                    </SelectContent>
                  </Select>
                </div>
                  ...
                  <Select value={excelDesdeMes} onValueChange={setExcelDesdeMes}>
                    ...
                    <SelectContent>
                      {Array.from({ length: 12 }, (_, i) => {
                        ...
                        return <SelectItem key={`desde-mes-${mes}`} value={mes}>{mes}</SelectItem>;
                      })}
                    </SelectContent>
                  </Select>
                </div>
                  ...
                  <Select value={excelDesdeDia} onValueChange={setExcelDesdeDia}>
                    ...
                    <SelectContent>
                      {Array.from({ length: 31 }, (_, i) => {
                        ...
                        return <SelectItem key={`desde-dia-${dia}`} value={dia}>{dia}</SelectItem>;
                      })}
                    </SelectContent>
                  </Select>
                </div>
                ...
                <div className="flex flex-col">
                  ...
                  <Select value={excelHastaAnio} onValueChange={setExcelHastaAnio}>
                    ...
                    <SelectContent>
                      {Array.from({ length: 13 }, (_, i) => {
                        const year = new Date().getFullYear() + i;
                        return <SelectItem key={`hasta-anio-${year}`} value={year.toString()}>{year}</SelectItem>;
                      })}
                    </SelectContent>
                  </Select>
                </div>
                <div className="flex flex-col">
                  ...
                  <Select value={excelHastaMes} onValueChange={setExcelHastaMes}>
                    ...
                    <SelectContent>
                      {Array.from({ length: 12 }, (_, i) => {
                        const mes = (i + 1).toString();
                        return <SelectItem key={`hasta-mes-${mes}`} value={mes}>{mes}</SelectItem>;
                      })}
                    </SelectContent>
                  </Select>
                </div>
                <div className="flex flex-col">
                  ...
                  <Select value={excelHastaDia} onValueChange={setExcelHastaDia}>
                    ...
                    <SelectContent>
                      {Array.from({ length: 31 }, (_, i) => {
                        const dia = (i + 1).toString();
                        return <SelectItem key={`hasta-dia-${dia}`} value={dia}>{dia}</SelectItem>;
                      })}
                    </SelectContent>
                  </Select>
                </div>

...
export async function GET(request: Request) {
  try {
    ...
    const resultado = idsCumplido.map((id_cumplido) => {
      ...
      const colaborador_nombre = row.colaborador_nombre || '';
      const colaborador_apellido = row.colaborador_apellido || '';
      const colaborador = (colaborador_nombre + (colaborador_apellido ? ' ' + colaborador_apellido : '')).trim();
      return {
        id_cumplido,
